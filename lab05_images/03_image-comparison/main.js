...
// we are pre-loading images, otherwise I think this page would take too long to load and compare
...
// this is not called in anything ^^


// Find the most similar images and store them in an array
function findMostSimilarPair(images) {
  let mostSimilarPair = [];
  let lowestDistance = 1; // distance of 1 is maximum value, images are completely different
  // start from high and compare down
  images.forEach((image, index) => {
    images.slice(index + 1).forEach((otherImage, otherIndex) => {
      const distance = Jimp.distance(image, otherImage);
